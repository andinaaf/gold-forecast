...
def simple_lstm_simulation(df, look_back=12, n_epochs=50):
    """
    Simulasi LSTM sederhana menggunakan numpy (tanpa TF)
    untuk preview di dashboard. Model asli dijalankan di notebook.
    """
    ...
    # Simulasi prediksi: moving average + noise kecil (proxy)
    ...
    trend_monthly = (y_test_actual[-1] - y_test_actual[0]) / len(y_test_actual)
    future_vals = [last_val + trend_monthly * (i+1) + np.random.normal(0, 10) for i in range(n_future)]
...
with st.sidebar:
    ...
    look_back   = st.slider("Look-back Window (bulan)", 3, 24, 12, 1,
                            help="Jumlah bulan historis yang digunakan sebagai input")
    ...
    <b style='color:{TEXT}'>Framework:</b> TensorFlow / Keras<br>
...
with tab1:
    ...
    # Main chart: full history + prediksi
    ...
    # Prediksi test
    fig.add_trace(go.Scatter(
        x=result['idx_test'], y=result['y_test_pred'],
        name='Prediksi LSTM', mode='lines',
        line=dict(color=RED, width=2.5, dash='dash')
    ))

    # Forecast
    fig.add_trace(go.Scatter(
        x=result['future_dates'], y=result['future_preds'],
    ...
    fig.add_vrect(
        x0=result['idx_test'][0], x1=df.index[-1],
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor='rgba(255,255,255,0.06)', title='Tanggal',
                   tickformat='%b %Y'),
    ...
    if selected_vars:
        ...
        for i, var in enumerate(selected_vars, 1):
            fig2.add_trace(go.Scatter(
                ...
                line=dict(color=colors_map.get(var, GOLD), width=2),
                fill='tozeroy', fillcolor=colors_map.get(var, GOLD).replace('#', 'rgba(') + ',0.08)'
            ), row=i, col=1)
...
with tab3:
    ...
    m2.metric("RMSE", f"${metrics['RMSE']:.2f}", help="Root Mean Square Error")
    m3.metric("MAPE", f"{metrics['MAPE']:.2f}%", help="Mean Absolute Percentage Error")
    m4.metric("R² Score", f"{metrics['R2']:.4f}", help="Koefisien Determinasi")

    st.markdown('<div class="section-header">📊 Aktual vs Prediksi LSTM</div>', unsafe_allow_html=True)

    fig5 = go.Figure()
    fig5.add_trace(go.Scatter(
        x=result['idx_train'], y=result['y_train_actual'],
        name='Aktual (Train)', mode='lines',
        line=dict(color=MUTED, width=1.5)
    ))
    fig5.add_trace(go.Scatter(
        x=result['idx_test'], y=result['y_test_actual'],
        name='Aktual (Test)', mode='lines',
        line=dict(color=GOLD, width=2.5)
    ))
    fig5.add_trace(go.Scatter(
        x=result['idx_train'], y=result['y_train_pred'],
        name='Prediksi (Train)', mode='lines',
        line=dict(color='rgba(59,130,246,0.6)', width=1.5, dash='dot')
    ))
    fig5.add_trace(go.Scatter(
        x=result['idx_test'], y=result['y_test_pred'],
        name='Prediksi (Test)', mode='lines',
        line=dict(color=RED, width=2.5, dash='dash')
    ))
    fig5.add_vline(
        x=result['idx_test'][0], line_color=BLUE, line_dash='dot',
    ...
    with col_r1:
        ...
        fig_res.add_trace(go.Scatter(
            x=result['idx_test'], y=residuals, mode='lines+markers',
            line=dict(color=RED, width=1.5),
            marker=dict(size=5, color=RED),
            name='Residual', fill='tozeroy', fillcolor='rgba(239,68,68,0.1)'
        ))
    ...
    fig_sc.add_trace(go.Scatter(
        ...
        mode='lines', line=dict(color=GOLD, dash='dash', width=2), name='Garis Ideal'
    ))
    fig_sc.add_trace(go.Scatter(
        x=result['y_test_actual'], y=result['y_test_pred'],
    ...
    fig_sc.update_layout(
        title=dict(text=f'<b>Aktual vs Prediksi</b>  |  R² = {metrics["R2"]:.4f}',
...
with tab4:
    ...
    Untuk hasil forecast LSTM yang akurat, jalankan file <code>emas_lstm.ipynb</code> secara lengkap
    dan gunakan model yang telah dilatih (<code>model_lstm_emas.h5</code>).
    </div>
    """, unsafe_allow_html=True)
    ...
    fig_fc.add_trace(go.Scatter(
        x=conn_x, y=conn_y,
        name='Forecast LSTM', mode='lines+markers',
...
with tab5:
    ...
    # Comparison aktual vs prediksi (test set)
